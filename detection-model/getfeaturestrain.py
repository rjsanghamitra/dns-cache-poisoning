...
def main():
    ...
    cwd = os.getcwd()
    path=cwd+"/dataset/"
    df=pd.read_csv(path+"Webpages_Classification_train_data.csv")
    header = ["url", "ip_add", "url_len", "tld", "ttl", "query_class", "record_class", "Status_code", "ip_add", "ip_add", "ip_add", "ip_add", "ip_add"]  # Replace with your header names
    csv_file = "data.csv"

    # Create an empty DataFrame with headers and save it to a CSV file
    ...
    for url, ip, label in zip(df["url"], df["ip_add"], df["label"]):
        ...
        try:
            ...
            emptydf.append(path+"Train_Data.csv")
        except Exception as e:
            continue
    ...
    # print(ans)
WWWER888888888888888888888888888888888888888888888888888888888888888888888888888
